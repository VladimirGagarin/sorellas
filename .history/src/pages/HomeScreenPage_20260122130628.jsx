...
export default function HomeScreenPage() {
  ...
  // Search filter
  const searchedFlowers = filteredFlowers.filter((flower) =>
    flower.name[language].toLowerCase().includes(searchTerm.toLowerCase()) ||
    flower.description[language].toLowerCase().includes(searchTerm.toLowerCase())
  );

  // Get random flower for featured section
  ...
  return (
    <div className="home-screen">
      ...
      <div className="home-content">
          ...
          <div className="featured-glass">
            <div className="section-header">
              ...
              <h2 className="section-title">
                {language === "en" ? "Flower of Inspiration" : "Fiore dell'Ispirazione"}
              </h2>
          ...
          <div className="grid-header">
            <h2 className="grid-title">
              ...
              <span className="flower-count">
                {searchedFlowers.length} {language === "en" ? "flowers" : "fiori"}
              </span>
            </h2>
        ...
        {/* Time-Based Sections (Alternative View) */}
        <section className="time-sections-glass">
          <div className="time-section-glass morning">
            <div className="time-section-header">
              <span className="time-emoji">🌅</span>
              <div>
                <h3>{language === "en" ? "Morning Dew" : "Rugiada Mattutina"}</h3>
                <p>
                  {language === "en"
                    ? "Prayers for new beginnings"
                    : "Preghiere per nuovi inizi"}
                </p>
              </div>
            </div>
            <div className="time-flowers-scroll">
              {flowers
                .filter(f => f.DayTime[language] === (language === "en" ? "Morning" : "Mattina"))
                .slice(0, 3)
                .map(flower => (
                  <FlowerCard
                    key={flower.id}
          ...
          <div className="time-section-glass midday">
            <div className="time-section-header">
              <span className="time-emoji">☀️</span>
              <div>
                <h3>{language === "en" ? "Noon Light" : "Luce di Mezzogiorno"}</h3>
                <p>
                  {language === "en"
                    ? "Prayers for strength and clarity"
                    : "Preghiere per forza e chiarezza"}
                </p>
              </div>
            </div>
            <div className="time-flowers-scroll">
              {flowers
                .filter(f => f.DayTime[language] === (language === "en" ? "Midday" : "Mezzogiorno"))
                .slice(0, 3)
                .map(flower => (
                  <FlowerCard
                    key={flower.id}
          ...
          <div className="time-section-glass evening">
            <div className="time-section-header">
              <span className="time-emoji">🌙</span>
              <div>
                <h3>{language === "en" ? "Evening Stars" : "Stelle della Sera"}</h3>
                <p>
                  {language === "en"
                    ? "Prayers for peace and reflection"
                    : "Preghiere per pace e riflessione"}
                </p>
              </div>
            </div>
            <div className="time-flowers-scroll">
              {flowers
                .filter(f => f.DayTime[language] === (language === "en" ? "Evening" : "Sera"))
                .slice(0, 3)
                .map(flower => (
                  <FlowerCard
                    key={flower.id}
