...
import { FaPray, FaChurch, FaHeart, FaArrowLeft, FaArrowRight, FaShareAlt, FaVolumeUp, FaBookmark, FaRegBookmark } from "react-icons/fa";
import { useLanguage } from "../contexts/useLanguage";
import "./LitanyPageTemplate.css";

export default function LitanyPageTemplate({ 
  title, 
  litany, 
  theme = "light" 
}) {
  ...
  const handlePlayAudio = () => {
    if ('speechSynthesis' in window) {
      const utterance = new SpeechSynthesisUtterance(currentItem[`title_${language}`]);
  ...
  return (
    <div className={`litany-page ${theme}`}>
      ...
      <div className="litany-content">
        <div className="litany-card">
          {/* Image Section */}
          <div className="litany-image-section">
            <div className="image-container">
              <img 
                src={currentItem.image} 
                alt={currentItem[`title_${language}`]}
              ...
              <button
                ...
                aria-label={
                  favorites.has(currentIndex) 
                    ? (language === 'en' ? 'Remove from favorites' : 'Rimuovi dai preferiti')
                    : (language === 'en' ? 'Add to favorites' : 'Aggiungi ai preferiti')
                }
              >
                {favorites.has(currentIndex) ? (
          ...
          <div className="nav-progress">
            <div className="progress-bar">
              <div 
                ...
                style={{ width: `${((currentIndex + 1) / litany.length) * 100}%` }}
              />
            </div>
