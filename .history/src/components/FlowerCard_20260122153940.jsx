...
export default function FlowerCard({
  ...
  const handlePray = () => {
    ...
    // navigate(`?pray=${flower.id}_${language}_${flower.name[language]}`, { replace: true });

    // Encode flower name to handle spaces and special characters
    const encodedFlowerName = encodeURIComponent(flower.name[language]);

    // Create the pray parameter
    const prayParam = `${flower.id}_${language}_${encodedFlowerName}`;

    // Get current URL parameters
    const params = new URLSearchParams(window.location.search);

    // Set the pray parameter
    params.set("pray", prayParam);
  ...
  if (compact) {
    return (
      <div className={`flower-card ${compact ? "compact" : ""}`}>
        ...
        {/* Flower Image */}
        <div className="flower-image-container" ref={imageRef}>
          ...
          {isImageVisible && (
            <img
              ...
              className={`flower-image ${imageLoaded ? "loaded" : "loading"}`}
  ...
  return (
    <div
      className={`flower-card ${isDaily ? "daily" : ""}`}
      title={`${flower.prayer[language].substring(0, 60)}...`}
    >
      {isDaily && (
      ...
      <div className="flower-content">
        ...
        <div className="flower-actions">
          <button
            className="action-btn btn-pray"
            onClick={handlePray}
            title={`${flower.prayer[language].substring(0, 50)}...`}
          >
            <FaPray /> {language === "en" ? "Pray" : "Prega"}
          </button>
